...
- **`meralion_mlx/processor.py`** — `MERaLiONProcessor` handles audio and text prep. Uses HF `WhisperFeatureExtractor` for exact mel spectrogram match with training. Expands the single `<SpeechHere>` token (index 255999) to `100 × num_chunks` copies in the input_ids before embedding.
